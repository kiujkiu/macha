// mlkpai_carrier_disc — 转子承载盘 Φ200×6 (旧 rim_top_disc 保留)
// 16 挂环孔装 rim_ring; 6 Φ14 凸台(带铜螺母孔)固定 pi2hub; 3 Φ6 托顶连接器
disc_od=200; thick=6;
inner_r=35; outer_r=77.5;
m3=3.2; cb_d=7; cb_deep=2.5;              // 挂环孔 + 顶沉
boss_d=14; boss_h=2;                      // 6 安装凸台
thru_d=3.2; insert_d=4.2; insert_deep=4;  // 通孔 + 铜螺母沉孔(从凸台顶)
...
module mlkpai_carrier_disc() {
    difference() {
        ...
        // 6 凸台孔: Φ3.2 通 + Φ4.2 从凸台顶沉 4
        for (b=boss_xy) {
            translate([b[0],b[1],-1]) cylinder(h=boss_top+2, d=thru_d, $fn=32);
            translate([b[0],b[1],boss_top-insert_deep]) cylinder(h=insert_deep+1, d=insert_d, $fn=48);
        }
    }
}
mlkpai_carrier_disc();
