"""
螺纹试打件 threaded_post_d35x70_x2mm_thread_test (2026-09-15, 简单试打版的螺纹单独试打; 原名 threaded_post_d50x70_x2mm_thread_test)

= 同目录母件 build_stl.py (十字筋 / 厚度统一 2 / Φ35 封底 / 螺纹 Φ27.5/Φ24.5 / 螺纹柱实心) 在顶板底面 Z = BASE_H − PLATE_T 横切,
  只留上面:
  - 螺纹柱整段 (Z 70–85): 牙型 / 顶端倒角 C1.75 / 实心 与母件一致
  - 下部圆柱留 BASE_KEEP = 母件顶板厚 PLATE_T (2.0; 保证贴床面是实心整圆, 直径随母件 = Φ35)
几何直接复用母件 build_stl.py (exec 到临时目录, 母件的全部 assert 照跑, 它自己的 STL 写进临时目录后丢弃), 母件参数改了这里重跑即同步。
2026-09-15: 配合的是塑料/打印内螺纹; 用户已试打确认间隙 0 可拧上 ⇒ 只出母件自身间隙一个。
  需要再比较间隙时把 EXTRA_CLEARANCES 设为 (0.4,) 等 (只在 exec 时替换母件源码里的 THR_CLEAR 行, 母件文件不动)。
...
STEM = "threaded_post_d35x70_x2mm_thread_test"
...
def build(clear):
    ...
    _hdr = f"thread_test of threaded_post_d35x70_x2mm clr{clear:g} cut Z{Z_CUT:g} H{H:g}".encode("ascii")
