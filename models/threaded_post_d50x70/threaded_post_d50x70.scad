...
// 下部圆柱 Φ50 × 70 底部敞口 + 米字加强筋 + 45° 过渡 (腔顶无水平面, 免支撑); 上部外螺纹柱高 15。
// 改⑥ (2026-09-15): 设计载荷单件 50 kg, 第二轮受力分析定稿 FB: 筒壁 2.52 / 米字筋 3.36 / 顶板 2.52 / 45° 过渡 6.2 / 底边 C0.3;
//      螺纹直接按凸起 Φ27.5 / 凹陷 Φ24.5 建模 (不留间隙, 牙高 1.5), 顶端倒角 C1.75;
//      螺纹柱中空 (用户要求): 壁厚 3 从牙底量 ⇒ 内孔 Φ18.5 盲孔深 15。
//      ⚠ 根部复核 (2026-09-15): 3 mm 中空在 50 kg 冲击工况最低 SF 1.19 (<1.5, 实心 1.40), 未达设计目标; 用户决定保留中空。
// 历史: 实心 → 壁厚 5 空心 + 米字筋 → 45° 过渡免支撑 → 100 kg 定稿 F1 (壁 2.94 / 筋 3.36 / 顶板 3.36 / 螺纹 26.1/24.1 实心)。
// 过渡棱柱与 r = rc + wall/2 圆柱求交 (不裁则筋端过渡半径 ≈25.18, 凸出 Φ50 外壁)。
// 用户未指定、按常规补: 60° 对称梯形牙, 右旋单线, 顶端导向倒角。
...
thr_min = 24.5;   // 小径 (牙底)
pitch   = 3;      // 螺距
thr_clear = 0;    // 打印间隙 (直径) (F1 0.4; 改⑥ 直接按 27.5/24.5 建模)
bot_ch    = 0.3;  // 底边外圈 45° 倒角, 防首层象脚
wall       = 2.52;  // 下柱筒壁厚 (F1 2.94)
plate_t    = 2.52;  // 下柱腔顶顶板厚 (F1 3.36)
rib_t      = 3.36;  // 米字加强筋厚
rib_angles = [0, 45, 90, 135];   // 米字筋; 筋和过渡共用
stud_wall  = 3;     // 螺纹柱壁厚, 从牙底 (小径) 量到内孔 (改⑥ 用户要求中空 3 mm)
bore_d     = thr_min - 2 * stud_wall;   // 螺纹柱内孔直径 18.5 (F1 实心)
bore_depth = thr_h; // 盲孔深 15, 孔底在台阶面 Z=70
rib_l   = base_d - wall;   // 筋条总长 = 腔半径 + 外半径, 两端伸进壁厚中线
gusset  = 6.2;    // 筋两侧/筒壁与腔顶 45° 过渡直角边 (F1 6), 须 > 格内切圆半径 (≈5.007) + 0.5 ⇒ 腔顶无水平面
zc      = base_h - plate_t;    // 腔顶 67.48
rc      = base_d / 2 - wall;   // 腔半径 22.48
r_gclip = rc + wall / 2;       // 45° 过渡裁剪圆柱半径 = 筒壁中线 23.74
e       = 0.5;    // 过渡三角形外扩进筋/壁/顶板, 免共面

flank_angle = 60;   // 牙型角
chamfer     = 1.75; // 顶端 45° 倒角, 0 = 不倒 (F1 1.5; 牙高 1.5 时 C1.5 顶端半径 = 小径半径)
right_hand  = true;
n_sec       = 360;  // 横截面分段
...
module threaded_post_d50x70() {
    difference() {
        union() {
            ...
            // 45° 过渡 (筋两侧 + 筒壁一圈), 整体与 r = r_gclip 圆柱求交 (不裁则筋端过渡凸出外壁)
            intersection() {
                union() {
