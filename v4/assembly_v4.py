...
# 2026-08-01 (用户圈图指定, 覆盖 07-31 的"用板自带 4 安装孔"结论):
#   柱改坐**网格 M6 螺纹孔** (87.5,37.5) 族 —— 螺纹孔不用螺母, M6 双头螺柱
#   一头拧板一头拧柱底内丝, 板底完全不凸 (板自带的 4 个 Φ6.5+Φ11 沉安装孔就空着,
#   正好留给"把板锁死到桌面/机架"用)。
#   POST_R 106.066 → 95.197, 柱角向 45+90k → 23.199+90k。
#   ⚠ 柱内缘 91.197: 对罩/托盘 R85 余 6.20; 对 wifi 线缆母头扫掠 R90.51 只余 0.69。
#     用户确认「线的问题可以处理」—— 装机时线必须往内收, 不能让母头甩到 R>91。
POST_H = 290.0
POST_GRID = (87.5, 37.5)
...
FRAME_DIR = ROOT / "models/top_bearing_v4"
