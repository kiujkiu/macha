...
for k in range(N_HOLES):
    ...
    if OUTER_CUTOUT_ENABLE and OUTER_CUT_A_S - 0.5 <= ang_d <= OUTER_CUT_A_E + 0.5:
        continue
    a = math.radians(ang_d)
...
text(sa_t_zero_x, 50, "剖视图  Section A-A  (1:1)   尺寸单位: mm",
     size=TXT_L, anchor="middle")
text(sa_t_zero_x, 57,
     ("(沿 0°–180° 轴剖切; +t = 0° 过槽口, -t = 180°)" if SLOT_ENABLE
      else "(沿 0°–180° 轴剖切; v4 无开口, 左右对称实体)"),
     size=TXT_I, anchor="middle")
...
if SLOT_ENABLE:
    ...
    line(*sa(R_BI, 0), *sa(R_BO, 0), GEOM_W)                    # 底面
    line(*sa(R_BI, BASE_T), *sa(R_IBO, BASE_T), GEOM_W)         # 底盘顶 (内)
    line(*sa(R_OBI, BASE_T), *sa(R_BO, BASE_T), GEOM_W)         # 底盘顶 (外)
    line(*sa(R_BO, 0), *sa(R_BO, TOTAL_H), GEOM_W)              # 外缘
    line(*sa(R_BI, 0), *sa(R_BI, TOTAL_H), GEOM_W)              # 内孔
    line(*sa(R_IBO, TOTAL_H), *sa(R_BI, TOTAL_H), GEOM_W)       # 内凸台顶
    line(*sa(R_IBO, BASE_T), *sa(R_IBO, TOTAL_H), GEOM_W)       # 内凸台外壁
    line(*sa(R_BO, TOTAL_H), *sa(R_OBI, TOTAL_H), GEOM_W)       # 外凸台顶
    line(*sa(R_OBI, BASE_T), *sa(R_OBI, TOTAL_H), GEOM_W)       # 外凸台内壁
