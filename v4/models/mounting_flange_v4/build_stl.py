...
# ===== v4 (2026-07-30, 用户: 外层凸台的缺口去掉) =====
# 原有两个 5° 楔形缺口, 只切外圈凸台 (R82.5..85, Z3..10), 基盘不动:
#   CUT1 -5°..0°   CUT2 -45°..-40°
# ⚠ 脚本 docstring 里写的 "0..10° / 40..50°" 是过期的, 实际参数是下面这两个。
# 这两个与 flange_disc 的开口是**配对**的 —— mounting_flange 在装配里绕 X 翻转
# (y→-y), 故 -5..0 翻成 0..+5 对上法兰盘的走线槽, -45..-40 对上外圈凸台缺口。
# flange_disc_v4 已把走线槽收窄到 R40..60 (不再通到盘缘) 且外圈缺口整个去掉,
# 这两个缺口已无对应物 → 一并关掉, 外圈凸台恢复整圈。
CUTOUT_ENABLE = False
CUT1_A_S = -5.0
CUT1_A_E = 0.0
CUT2_A_S = -45.0
...
for a_s, a_e in (((CUT1_A_S, CUT1_A_E), (CUT2_A_S, CUT2_A_E)) if CUTOUT_ENABLE else ()):
