...
m3_diam         = 3.2;
cb_diam         = 7;
inner_cb_enable = true;  // 内圈 (PCD Φ72.5) 沉孔 — 2026-09-07 用户「恢复, 深度 2mm」
outer_cb_enable = true;
cb_depth_inner  = 2;    // 2026-09-07 恢复成 2 (曾 0.5, 曾整个取消) ⇒ 与外圈同规格
...
cutout_enable   = false;   // v4 (2026-07-30): 外圈凸台两个 5° 缺口去掉 → 整圈
cut1_a_s        = -5;
cut1_a_e        = 0;
cut2_a_s        = -45;
cut2_a_e        = -40;
            ...
            translate([0, 0, base_t]) difference() {
                ...
                translate([0, 0, -0.1])
                    linear_extrude(height = boss_h + 0.2)
                        if (cutout_enable) pie_wedge(cutout_r, cut1_a_s, cut1_a_e);
                // cutout 2: 40°-50°
                translate([0, 0, -0.1])
                    linear_extrude(height = boss_h + 0.2)
                        if (cutout_enable) pie_wedge(cutout_r, cut2_a_s, cut2_a_e);
            }
        }
