"""
A3 drawing — frame_B_v4 (v4 顶轴承架 B, 上层架, PLA ×1)。
v4 (2026-08-01): 柱改坐网格 M6 螺纹孔 (87.5,37.5) 族 → POST_R 106.066→95.197,
柱角向 45+90k → 23.199+90k; 除半径/角向外与 v3 完全相同 (筋孔 r36.7/46.7 不动)。
...
text(PAGE_W/2,15,"POV 3D v4 — frame_B_v4 顶轴承架 B (上层架, ×1)",size=TXT_T,anchor="middle")
...
# 尺寸线偏到筋右侧固定 6.07 (v4: POST_R 变小 → XMID 左移, 原来写死的 117.0 会翻到左侧压住箭头)
vdim(fv(0,VANE_BOLT_Z)[1],fv(0,0)[1],fv(RIB_W/2,0)[0],fv(RIB_W/2,0)[0]+6.07,f"{VANE_BOLT_Z:g}")
# 引注
...
text(87.0,172.6,"M3×20+螺母 ×2 挂挡光滑片 vane_slider_v3 (孔位不变, 滑片沿用不重印)",size=TXT_I)
...
text(tb_x+4,tb_y+6,"POV 3D v4 结构件 — frame_B_v4 顶轴承架 B (上层, ×1, PLA; 翻转姿态打印零支撑)",size=TXT_L)
text(tb_x+tb_w-4,tb_y+6,"投影 1st-angle / 比例 见各视图",size=TXT_I,anchor="end")
text(tb_x+4,tb_y+14.5,f"毂 Φ{HUB_D:g} (z8..16) / 臂 {ARM_W:g}×{ARM_T:g} ×2 / 筋 {RIB_W:g}×{RIB_T:g} (z0..8, 底平垫柱底) / 垫柱 Φ{PAD_D:g}×{Z_TOP:g} @R{POST_R:.2f} (12.5·√58, 柱落网格螺纹孔 ({POST_GRID[0]:g},{POST_GRID[1]:g}) 族) / "
     ...
     f"装配 rot{POST_A0+270:.3f}° 置柱顶: 垫/筋底 asm290, 毂 298..306, 688 上轴承 301..306 / 总高 {Z_TOP:g} / 足迹 {FOOT:.2f}×{FOOT:.2f} / 单位 mm",size=TXT_I)
text(tb_x+tb_w-4,tb_y+14.5,"2026-08-01  /  POV3D / v4 / top_bearing_v4 / frame_B_v4.stl",size=TXT_I,anchor="end")

out = Path(__file__).with_name("frame_B_v4_drawing.pdf")
try:
    pdf.output(str(out)); print(f"wrote {out}")
except PermissionError:
    alt = Path(__file__).with_name("frame_B_v4_drawing.NEW.pdf"); pdf.output(str(alt)); print(f"wrote {alt} (locked)")
