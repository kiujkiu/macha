"""
A3 drawing — vane_slider_v4 (光电挡光滑片 (左右对称版, 孔/刀片/板同心于 X=43.7), 刀片印长装机剪短版, 锁 frame_B 45°臂筋, 2026-08-24)。
...
from build_vane_slider import (PLATE_X0, PLATE_X1, PLATE_Y0, PLATE_Y1, PLATE_Z1, SYM_C, R_LO, R_HI,
...
text(PAGE_W/2,15,"POV 3D v3 — vane_slider_v4 光电挡光滑片 (×1, 刀片印长装机剪短)",size=TXT_T,anchor="middle")
...
text(tb_x+4,tb_y+6,"POV 3D v3 结构件 — vane_slider_v4 光电挡光滑片 (×1, PLA, 刀片印长装机剪短)",size=TXT_L)
...
text(tb_x+tb_w-4,tb_y+14.5,"2026-07-24  /  POV3D / v3 / photo_sensor_v3 / vane_slider_v4.stl",size=TXT_I,anchor="end")

out = Path(__file__).with_name("vane_slider_v4_drawing.pdf")
try:
    pdf.output(str(out)); print(f"wrote {out}")
except PermissionError:
    alt = Path(__file__).with_name("vane_slider_v4_drawing.NEW.pdf"); pdf.output(str(alt)); print(f"wrote {alt} (locked)")
