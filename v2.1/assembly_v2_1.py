"""
POV3D 装配 v2.1 (assembly_v2_1) —— 基于 v2, 唯一改动 (2026-07-16):
四周固定柱从网格 (±125,±125) 向中心各移 1 格 (25mm) → (±100,±100),
POST_R 176.78→141.42; frame_A/B 换 v2.1 短臂版 (models/top_bearing_v2_1)。
其余与 v2 完全一致。转子最大回转半径 ~114.7 (光电支架角) vs 柱内缘 137.4, 隙 22.7。
...
ROOT = Path(__file__).parent            # pov3d/v2.1
...
#    (落座面 = 托盘顶+5 = 47.2)。 PCB 方向 PCB_ROT + 挪位 PCB_OFF 不变。
PCB_ROT = 90.0
...
gb = np.concatenate([read_stl(MODELS / "l_bracket_v2/gantry_base_A.stl"),
                     read_stl(MODELS / "l_bracket_v2/gantry_base_B.stl")], axis=0) \
     + np.array([0.0, 0.0, DISC_TOP])
gb = rot_z(gb, ROTOR_ROT + SCREEN_FLIP)
parts.append(("gantry_base A+B", gb))
sp = read_stl(MODELS / "l_bracket_v2/screen_plate.stl") + np.array([0.0, 0.0, DISC_TOP])
...
sc = read_stl(MODELS / "screen_150x169/screen_150x169.stl") + np.array([0.0, 0.0, SCREEN_Z0])
...
#     贴盘, 14.5 高; 放平模块孪生待重建, 暂不画)。出口窗朝盒长轴 + 端。
#     零件系 → 盘系: 绕 Y +90° ((x,y,z)→(z,y,−x), part X 深度变竖直), 开口面落
#     Z=0, 长轴 (part Y, 含沿 96.6) 沿盘 Y; footprint (46.4 向) 中线挪到 XC_WIFI;
#     随 135° 组转后长边与 pi2hub 板边平行 (板长边 PCB_ROT=90+135 沿 45° 线,
#     盒长轴 90°+135° 同向)。外角 r=hypot(69.2,48.3)=84.4 < R85。
#     4× 沿孔落盘系 (33.5,±43.3)/(58.5,±43.3) 再转 135° —— rim_ring 已开同位
#     4 孔 (2026-07-21 晚, Φ3.2 通 + Φ4.2×4.5 唇侧铜螺母沉孔, M3×8 上装)。
...
ws = read_stl(ROOT.parent / "models/usb_wifi/wifi_shell.stl")
...
#     M6×40 平头 (头窝板底; v2.1 帽板减薄 9→6, 板底 286.7, 杆身 289.4..329.4)
#     拧进 Φ8×50 M6内丝螺柱 (319.4..369.4, 顶锁 M6 螺母) 穿双 688 (353..366)。
CAPTOP_V2 = 292.7
POST_H, POST_XY = 350.0, 100.0   # v2.1: 125→100 (内移 1 格)
...
fa = read_stl(ROOT / "models/top_bearing_v2_1/frame_A_v2_1.stl")
fa = rot_z(fa, 135.0)
fa[..., 2] += POST_H
parts.append(("frame_A_v2 (SW+NW)", fa))
fb = read_stl(ROOT / "models/top_bearing_v2_1/frame_B_v2_1.stl")   # 打印翻转姿态
fb[..., 1] = -fb[..., 1]
fb[..., 2] = POST_H + 16.0 - fb[..., 2]
fb = fb[:, ::-1, :].copy()
fb = rot_z(fb, -45.0)
parts.append(("frame_B_v2 (NE+SE)", fb))
cap = read_stl(ROOT / "models/top_cap_v2_1/top_cap_v2_1.stl")   # 打印翻转姿态 (v2.1: 配重单排@25)
cap[..., 1] = -cap[..., 1]
cap[..., 2] = CAPTOP_V2 - cap[..., 2]
cap = cap[:, ::-1, :].copy()
cap = rot_z(cap, ROTOR_ROT)
parts.append(("top_cap_v2 (rotor)", cap))
scr = m3d.Manifold.cylinder(40.0, 3.0, 3.0, 32, False).translate((0, 0, 289.4))
parts.append(("M6x40 screw (rotor)", mesh_tris(scr)))
sto = m3d.Manifold.cylinder(50.0, 4.0, 4.0, 48, False).translate((0, 0, 319.4))
...
print(f"顶轴承 v2.1: 柱顶 {POST_H:.0f}; 帽板 {CAPTOP_V2-6:.1f}..{CAPTOP_V2:.1f} (减薄 9→6); "
      f"轴承 353..358 / 361..366; 螺柱 319.4..369.4")
...
out = ROOT / "assembly_v2_1.stl"
_header = b"POV3D assembly_v2_1"
...
fig.tight_layout()
png = ROOT / "assembly_v2_1_preview.png"
