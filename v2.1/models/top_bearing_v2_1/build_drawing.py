...
(GB first-angle, layout/helpers carried over from build_drawing.py sheet 1):
  top view (1:1, POST_R 141.42) + 688 press-pocket DETAIL A (4:1)
...
v2.1 (2026-07-16): four Φ8×350 posts moved 1 grid inward → (±100,±100)
→ POST_R = 100·√2 = 141.421 (v2: 176.777); ribs R24..R128.64. All else identical.
...
PAD_D = 18.0
POST_R = 100.0 * math.sqrt(2.0)          # 141.421 (v2.1)
RIB_R0 = 24.0
RIB_R1 = POST_R - 12.78
M6_CLEAR = 6.5
M3_TIGHT = 3.2
...
def title_block(line1, line2):
    ...
    text(tb_x + tb_w - 4, tb_y + 14.5, "2026-07-16 / POV3D / v2.1 / models / top_bearing_v2_1", size=TXT_I, anchor="end")
def save(name):
...
# ============================================================
text(PAGE_W/2, 14, "POV 3D top_bearing frame v2.1 — frame_A_v2_1 / frame_B_v2_1  (静止轴承架, 柱位内移1格, PLA)",
...
text(*fv(40, 155), "柱位 = 网格 (±100,±100) (v2.1 内移1格), R = 100×√2 ≈ 141.42", TXT_I, "start", True)
...
title_block("POV 3D — top_bearing frame v2.1 / frame_A_v2_1 + frame_B_v2_1 (各 ×1) / PLA / 柱位 (±100,±100) Φ8×350 M6螺纹柱",
            "R柱 = 100×√2 ≈ 141.42 (v2.1 内移1格) / 厚度: 臂/毂 8, 筋 6 (总高 A=14, B=16), 柱垫 A=8/B=16 仅Φ6.5通孔 / 高度: 柱顶350, A 350-358 (轴承353-358), B 358-366 (轴承361-366) / "
            "BOM: 688×2, M6×16×2 (A垫), M6×30×2 (B柱垫), M3×20+螺母×4 / A 正打, B 反打")
save("top_bearing_frame_v2_1_drawing.pdf")
