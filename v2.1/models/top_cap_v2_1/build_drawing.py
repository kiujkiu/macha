...
SLAB_T = 6.0
TP_X0, TP_X1, TP_HW = -36.0, 17.27, 72.0       # slab 53.27 x 144 (v2.1 裁短)
...
# ===== header =====
text(PAGE_W/2, 14, "POV 3D v2.1 — top_cap_v2_1  (v2 转子帽, 单屏转子, L型, PLA)",
     size=TXT_T, anchor="middle")
text(PAGE_W/2, 19.5,
     f"L型 (腿在+X端) / 平板 {TP_X1-TP_X0:g}×{2*TP_HW:g}×{SLAB_T:g} (asm X-36..+17.27, Y±72, Z286.7..292.7, 顶面全平; v2.1 裁短+减薄9→6) + 背板腿 {LEG_X1-LEG_X0:g}×{2*LEG_HW:g} 下至Z247 / "
     f"中心Φ{M6_BORE:g}通+底面头窝Φ{HEAD_D:g}×{HEAD_DEPTH:g} / 配重 单排 10×Φ{CW_M6:g} 直通 @X-25 (间距{CW_TRI:g}; v2.1: 46→25 单排, 沉孔取消) / 腿 2×Φ{M3_CLEAR:g}→屏上排螺母 (M3×16)",
...
for (cx, cy) in CW_A + CW_B:
    pdf.circle(*gv(cx, cy), CW_M6/2, style="D")   # 直通, 无沉孔
...
hdim(gv(CWA_X, 0)[0], gv(CWA_X, CW_TRI)[0],
     gv(CWA_X, 0)[1], gv(TP_X0, 0)[1] + 8, f"{CW_TRI:g}")
vdim(gv(0, 0)[1], gv(CWA_X, 0)[1], gv(0, TP_HW)[0], gv(0, TP_HW)[0] + DIM_O1, "25")
# notes under the plan
text(gx0, 171, f"配重 单排 10×Φ{CW_M6:g} 直通 @ X=-25, Y=(k-4.5)×14 (无沉孔, M6头凸出顶面)",
...
segs = [
    # top face, broken by the CW hole and the axis bore
    ((TP_X0, ZT), (CWA_X - cr, ZT)), ((CWA_X + cr, ZT), (-br, ZT)),
    ((br, ZT), (TP_X1, ZT)),
    # CW hole: Φ6.5 plain through (沉孔取消 2026-07-16)
    ((CWA_X - cr, ZT), (CWA_X - cr, ZB)), ((CWA_X + cr, ZT), (CWA_X + cr, ZB)),
    ...
    ((TP_X0, ZB), (CWA_X - cr, ZB)), ((CWA_X + cr, ZB), (-hr, ZB)),
    ((hr, ZB), (LEG_X0, ZB)),
...
pdf.line(*pv(CWA_X, ZT + 2), *pv(CWA_X, ZB - 2))
...
# --- dims ---
hdim(pv(TP_X0, 0)[0], pv(0, 0)[0], pv(0, ZT)[1], pv(0, ZT)[1] - 8.6, "36")   # -X edge to axis
...
text(dbx - 6, 265, "M6×40 平头自底面入 (头 286.7..289.4)", size=TXT_I)
...
text(nx, 226, "配重: M6 自顶面直通穿入 + 螺母, 头凸出板顶 (无沉孔), 逐孔配平", size=TXT_I)
...
text(tb_x + 4, tb_y + 6, "POV 3D — v2.1 / top_cap_v2_1 (转子帽, 配重单排@25) (×1)",
     size=TXT_L, anchor="start")
text(tb_x + tb_w - 4, tb_y + 6, "投影 1st-angle / PLA / 单位 mm", size=TXT_I, anchor="end")
text(tb_x + 4, tb_y + 14.5,
     "PLA / 打印姿态: 平板顶面朝下 (腿朝上, 免支撑) / L型 (腿在+X端) / 平板6厚 53.27×144 (裁短+减薄), 腿4厚 高45.7, 配重孔直通无沉 / "
     "BOM: M6×40平头, Φ8×50螺柱, M3×16 ×2, M6平头配重+螺母按需", size=TXT_I, anchor="start")
text(tb_x + tb_w - 4, tb_y + 14.5, "2026-07-16 / POV3D / v2.1 / models / top_cap_v2_1",
     size=TXT_I, anchor="end")

out = Path(__file__).with_name("top_cap_v2_1_drawing.pdf")
try:
