...
PAD_WI, PAD_WO = 6.7, 10.0             # 梯形半宽: 内边 ±6.7 (=屏厚13.4), 外边 ±10
...
part -= m3d.Manifold.cylinder(PAD_T + 2, SCR_D/2, SCR_D/2, 32, False)\
    .translate((0.0, SCR_Y, PAD_Z0 - 1.0))                             # 屏螺丝孔
part -= m3d.Manifold.cylinder(4.0, 7.5/2, 7.5/2, 32, False)\
    .translate((0.0, SCR_Y, PAD_Z0 - 4.0))                             # Φ7.5×4 帽让位窝

mesh = part.to_mesh()
verts = np.asarray(mesh.vert_properties)[:, :3]
tris = np.asarray(mesh.tri_verts)
out = Path(__file__).with_name("portal_tee_v3.stl")
with out.open("wb") as f:
    f.write(b"POV3D portal_tee_v3 (WIP)".ljust(80, b" "))
...
print(f"  梯形加强壁 (厚 {GUS_T:g}, 齐外侧面, ±{BAR_X:g}→±{PAD_WO:g}); 顶平板托梯形 外{2*PAD_WO:g}/内{2*PAD_WI:g}×深{THK_Y1-PAD_Y0:g}×{PAD_T:g} (托面 Z{STEM_Z1:g}=屏底), 屏孔 Φ{SCR_D:g}@(0,{SCR_Y:g}) + Φ7.5×4 帽让位窝")
