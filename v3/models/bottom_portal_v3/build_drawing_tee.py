"""
A3 drawing — portal_tee_v3 (v3 屏幕底部支撑 T 型件, 2026-07-22 深夜终版:
...
text(PAGE_W/2,15,"POV 3D v3 — portal_tee_v3 屏幕底部支撑 T 型件 (×2, 180° 对放)",size=TXT_T,anchor="middle")
text(PAGE_W/2,21,f"底横条 67×10×5 装转子两颗 M3 (±29.658, R77.5 环孔, Φ{WELL_D:g} 工艺井) / 竖梃 5×10 / 梯形加强壁 厚 {GUS_T:g} 齐外侧面 (±33.5→±10) / "
     f"顶平板托梯形 外{2*PAD_WO:g}·内{2*PAD_WI:g} (=屏厚13.4)×深{THK_Y1-PAD_Y0:g}×{PAD_T:g}, 托面 Z{STEM_Z1:g}=屏底一整平面, 屏孔 Φ{SCR_D:g} (M3×12 自下) / "
...
drect(*fv(-SCR_D/2,STEM_Z1),*fv(SCR_D/2,PAD_Z0))      # 屏孔 (隐藏, Y64)
...
note(*fv(PAD_WO,47.5),fv(45,0)[0],fv(0,42)[1],f"顶平板托 (梯形), 屏孔 Φ{SCR_D:g} (M3×12 自下)",anchor="start")
...
text(tb_x+4,tb_y+6,"POV 3D v3 结构件 — portal_tee_v3 (×2, 180° 对放; 托面 Z50 = 屏幕安装高度)",size=TXT_L)
...
text(tb_x+tb_w-4,tb_y+14.5,"2026-07-22  /  POV3D / v3 / bottom_portal_v3 / portal_tee_v3.stl",size=TXT_I,anchor="end")

out = Path(__file__).with_name("portal_tee_v3_drawing.pdf")
try:
