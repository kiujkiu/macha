...
(随架, = 筋底 asm 290)。
  ...
  · 板: X 40.3..56.3 (宽 16) × Y 2..6 (厚 4) × Z 0..8 —— 恰好填满筋侧面
    (2026-07-24 四改, 用户: 板顶到底 → 底与筋底平 asm 290, 顶抵臂底
    asm 298 成定位靠面, 装时向上顶到位+防转); 2×Φ3.2 圆孔在板正中
    (X 43.3 / 53.3 孔距 10, Z 4) 对齐筋孔; 板底对旋转叉顶 285.65 余 4.35+δ
  · 刀片: X 46.76..49.44 (厚 2.68), 印长 42 (Z −42..0, 全件总长 50);
    切向偏置 2..6 按真实半径修正 → 四角半径恰 46.80..49.80 (扫掠净通道
    44.7..51.9 内, 双侧余量 ~2.1); 剪短后刀尖 asm 280 (光轴 282.4)
打印: 平躺 (Y=2 面贴床), 足迹 16×50, 高 4, 零支撑。
STL 为打印姿态: (x, y, z)_件 → (x, z, y−2)_打印。
另出 vane_slider_v3_asm.stl (已剪短孪生, 刀尖 280) 供装配/干涉校核。
...
PLATE_X0, PLATE_X1 = 40.3, 56.3
PLATE_Y0, PLATE_Y1 = 2.0, 6.0
PLATE_Z1 = 8.0                       # = 筋高, 底平筋底 / 顶抵臂底 (定位靠面)
HOLE_XS, HOLE_D = (43.3, 53.3), 3.2   # 孔距 10, 对称于刀片中心 48.3
HOLE_Z = 4.0                         # 板正中, 对筋孔 z4 (asm 294) → 板底 asm 290
BLADE_X0, BLADE_X1 = 46.76, 49.44    # 切向偏置修正: 半径恰 46.80..49.80
...
# 四角真实半径校核
for (bx, by) in ((BLADE_X0, PLATE_Y0), (BLADE_X0, PLATE_Y1), (BLADE_X1, PLATE_Y0), (BLADE_X1, PLATE_Y1)):
    r = (bx*bx + by*by) ** 0.5
    assert 46.79 <= r <= 49.81, r
...
write_stl(part, "vane_slider_v3.stl", "POV3D vane_slider_v3 v3 (print, blade 50 trim-to-fit)")
write_stl(part_asm, "vane_slider_v3_asm.stl", "POV3D vane_slider_v3 asm twin (trimmed, tip 280)")
...
print(f"  刀片 X{BLADE_X0:g}..{BLADE_X1:g} 印长 {-BLADE_Z0:g} (全件总长 {PLATE_Z1-BLADE_Z0:g}); 装机剪短, 目标: 刀尖距压条顶 12mm (孪生剪至 {-TRIM_Z0:g})")
