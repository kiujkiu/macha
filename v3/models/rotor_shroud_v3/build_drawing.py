"""
A3 drawing — rotor_shroud_v3 转子电路罩 (两半对开, 一张图管 A/B 两件)。
...
text(PAGE_W/2, 14, "POV 3D v3 — rotor_shroud_v3 转子电路罩 (两半对开; 本图管 A/B 两件)",
...
text(PAGE_W/2, 25, f"屏缝: ±{SCR_HW:g} (|Y|≤{TEE_Y0:g}) → ±{TEE_HW:g} ({TEE_Y0:g}..{SLOT_Y1:g}, 让 portal_tee 梯形顶托 20 宽); "
...
# 顶板内边界 = 屏缝轮廓 (半 A 部分)
slot_edge = [pv(SCR_HW, SEAM_GAP), pv(SCR_HW, TEE_Y0), pv(TEE_HW, TEE_Y0),
             pv(TEE_HW, SLOT_Y1), pv(-TEE_HW, SLOT_Y1), pv(-TEE_HW, TEE_Y0),
             pv(-SCR_HW, TEE_Y0), pv(-SCR_HW, SEAM_GAP)]
polyline(slot_edge)
# 顶板与筒壁交线 (R82 已画) + 接缝处顶板边
polyline([pv(SCR_HW, SEAM_GAP), pv(R_IN, SEAM_GAP)], GEOM_W)
polyline([pv(-R_IN, SEAM_GAP), pv(-SCR_HW, SEAM_GAP)], GEOM_W)
# 屏缝下翻边 (隐藏, 在顶板之下)
for sgn in (1.0, -1.0):
    x0, x1 = sorted((sgn*SCR_HW, sgn*(SCR_HW+LIP_T)))
...
hdim(pv(-SCR_HW, 0)[0], pv(SCR_HW, 0)[0], pv(0, TEE_Y0/2)[1], TYC+30, f"{2*SCR_HW:g} 屏缝")
...
# ===================== A / B 差异说明 =====================
bx, by = 20.0, 236.0
_w(0.3); pdf.rect(bx, by, 190.0, 26.0, style="D")
...
text(tb_x+4, tb_y+14.5, f"Φ{OD:g}/Φ{ID:g}×{H:g}, 壁 {WALL:g}, 顶板 {PLATE_T:g} / 每件 ~78 cm3 (约 96 g PLA) / "
     f"BOM: M3×12 (头 Φ7.5) ×4 → hub 底既有铜花螺母 / 打印: 顶板贴床, 零支撑 / 单位 mm", size=TXT_I)
text(tb_x+tb_w-4, tb_y+14.5, "2026-07-27  /  POV3D / v3 / rotor_shroud_v3 / shroud_half_A_v3.stl + shroud_half_B_v3.stl",
     size=TXT_I, anchor="end")

out = Path(__file__).with_name("rotor_shroud_v3_drawing.pdf")
try:
